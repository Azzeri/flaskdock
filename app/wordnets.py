...
class Wordnets:
    ...
    def apply_recommendation_mechanism(self):
        ...
        sorted_documents = sorted(documents_with_semantic_similarity,
        ...
        for keyword in document['keywords']:
            for interest in self.user_interests:
                ...
                if self.synsets_exist(keyword_synsets, interest_synsets):
                    similarity_scores.append(self.get_similarity_scores_for_every_synsets_pair(
    ...
    def get_similarity_scores_for_every_synsets_pair(self, keyword_synsets, interest_synsets):
        wp_similarities = []
        for keyword_synset in keyword_synsets:
            for interest_synset in interest_synsets:
                if keyword_synset.pos() == interest_synset.pos():
                    wp_similarity = interest_synset.wup_similarity(
                        keyword_synset)
