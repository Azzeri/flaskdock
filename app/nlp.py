...
class Nlp:
    ...
    def filter_parts_of_speech(self, text):
        ...
        return [word for word, pos in pos_tags if pos.startswith(
            'N') or pos.startswith('J')]

    def get_top_keywords(self, keywords):
        freq_dist = FreqDist(keywords)
        return [keyword for keyword, _ in freq_dist.most_common(self.NUMBER_OF_KEYWORDS)]
